<template>
	...
	>
		<view class="u-calendar">
			...
			<view class="u-calendar__content" @touchstart="gtouchstart" @touchmove="gtouchmove" @touchend="touchend" :style="[{left:lefts+'rpx',top:tops+'rpx'}]">
				<!-- 前置空白部分 -->
				...
				>
					<view class="u-flex u-row-between" :style="[{ fontSize: '20rpx', color: !isStartEnd(index) ? activeBgColor : '#ffffff' }]">
						<text style="">{{ getPrice(index) }}</text>
					</view>
					<view class="u-calendar__content__item__inner" :style="{ color: getColor(index, 2) }">
						<view>{{ index + 1 }}</view>
					</view>
					<view class="" :style="[{ fontSize: '20rpx', color: numColor(index) }]">{{ getNums(index) }}</view>
					<view
...
export default {
	...
	computed: {
		...
		numColor() {
			return index => {
				let day = index + 1;
				return this.isPast(day) || !this.getPrice(index) ? '#ccc' : this.activeBgColor;
			};
		},
		isStartEnd() {
			return index => {
		...
		getPrice() {
			return index => {
				...
				if (this.calendarPrice[day] && this.calendarPrice[day].price != undefined && this.calendarPrice[day].price > 0) {
					return `￥${this.calendarPrice[day].price}`;
				}
				return '';
			};
		},
	...
	methods: {
		getMonthHouse() {
			return new Promise((resolve, reject) => {
				this.$api.getMonthHouse({ year: this.year, month: this.month, store_id: this.store_id }).then(res => {
					if (res.code == 1) {
		...
		isPast(day) {
			if (
				...
				(this.year == this.nowYear && this.month < this.nowMonth) ||
				(this.year == this.nowYear && this.month == this.nowMonth && day < this.nowDay)
			) {
				return true;
			} else {
				return false;
		...
		init() {
			...
			this.day = this.nowDay = now.getDate();
		...
		dateClick: function(day) {
			if (!this.getPrice(day)) {
				this.$u.toast('该日期没有房间了');
				return;
			}
			day += 1;
			if (this.isPast(day)) {
				this.$u.toast('不能选择过去的日期');
				return;
			}
		...
		touchend() {
			if(!this.direction || !this.firstDirection){
				return;
			}
			//结束触摸事件
			setTimeout(() => {
				...
				if(this.lefts>0 || this.tops<0){
					this.changeMonthHandler(0);
				}else{
					this.changeMonthHandler(1)
				}
				this.lefts = 0;
				this.tops = 0
			}, 100);
		...
		gtouchmove(e) {			
			...
			if (Math.abs(this.X) > Math.abs(this.Y) && this.X > 0) {
				...
				//第一次方向
				if(!this.firstDirection){
					this.firstDirection = 'bottom';
				}
				this.direction = 'bottom';				
			} else if (Math.abs(this.Y) > Math.abs(this.X) && this.Y < 0) {
				//第一次方向
				if(!this.firstDirection){
					this.firstDirection = 'top';
				}
				this.direction = 'top';				
			} else {
				console.log('touch');
